...
const NavigationItem: FC<NavigationItemProps> = ({ option, isActive, onClick }) => {
  ...
  const className = `nav-item block w-full text-left py-3 px-4 rounded-lg transition-colors duration-200 md:w-auto md:text-center md:py-2 md:rounded-full ${
    isActive && !isExternalPage
      ...
      : "text-ink hover:bg-white/45 md:hover:bg-white/20"
  }`;
...
export const Navigation: FC<NavigationProps> = ({
  ...
  return (
    <header className="fixed top-3 left-1/2 z-10 w-full max-w-xs -translate-x-1/2 transform px-3 md:max-w-md lg:max-w-lg">
      <div className="relative overflow-hidden rounded-full bg-white/45 p-4 backdrop-blur transition-all duration-300 md:px-6">
        <div className="flex items-center justify-between">
          ...
          {showMobileUI && (
            <button
              ...
              className="text-ink flex flex-col items-center justify-center space-y-1"
        ...
        {showMobileUI && (
          <div
            className={`absolute top-full right-0 left-0 mt-2 transform overflow-hidden rounded-2xl bg-white/45 backdrop-blur transition-all duration-300 ${
