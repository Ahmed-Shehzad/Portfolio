...
import type { ResumeType } from "@/features/resume";

interface SpecializedDownloadButtonProps {
  readonly resumeType: ResumeType;
}

export function SpecializedDownloadButton({ resumeType }: SpecializedDownloadButtonProps) {
  ...
  const handleDownload = async () => {
    try {
      ...
      // Call the specialized API endpoint for resume type
      const response = await fetch(`${baseUrl}/${currentLocale}/api/resume-pdf/${resumeType}`, {
        method: "GET",
        headers: {
      ...
      // Create download link with resume type in filename
      const url = window.URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.download = `muhammad-ahmed-shehzad-${resumeType}-resume-${currentLocale}.pdf`;
  ...
  return (
    <>
      ...
      <button
        ...
        aria-label={isGenerating ? "Generating PDF..." : `Download ${resumeType} resume as PDF`}
      >
        {isGenerating ? (
          <>
          ...
          <>
            ...
            <span>Download {resumeType.charAt(0).toUpperCase() + resumeType.slice(1)} Resume</span>
          </>
        )}
      </button>
    </>
