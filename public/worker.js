...
const sanitize = (input) => {
    const raw = typeof input === "string" ? input : String(input !== null && input !== void 0 ? input : "");
    // Build regex at runtime to avoid control character parsing issues
    const controlRanges = [
        [0x0000, 0x001f],
        [0x007f, 0x009f],
    ];
    const pattern = new RegExp(controlRanges
        .map(([a, b]) => {
...
const handlers = {
    [MESSAGE_TYPES.PROCESS_ANIMATIONS]: (data, id, start) => postResult(OUT_TYPES.ANIMATIONS_PROCESSED, processAnimationData(data), id, start),
    [MESSAGE_TYPES.OPTIMIZE_SCROLL_CALCULATIONS]: (data, id, start) => postResult(OUT_TYPES.SCROLL_OPTIMIZED, optimizeScrollCalculations(data), id, start),
    [MESSAGE_TYPES.CALCULATE_PERFORMANCE_METRICS]: (data, id, start) => postResult(OUT_TYPES.METRICS_CALCULATED, calculatePerformanceMetrics(data), id, start),
    [MESSAGE_TYPES.PROCESS_TESTIMONIALS]: (data, id, start) => postResult(OUT_TYPES.TESTIMONIALS_PROCESSED, processTestimonialsData(data), id, start),
    [MESSAGE_TYPES.OPTIMIZE_PROJECT_DATA]: (data, id, start) => postResult(OUT_TYPES.PROJECTS_OPTIMIZED, optimizeProjectData(data), id, start),
    [MESSAGE_TYPES.CALCULATE_STAR_RATINGS]: (data, id, start) => postResult(OUT_TYPES.STAR_RATINGS_CALCULATED, calculateStarRatings(data), id, start),
    [MESSAGE_TYPES.PROCESS_CONTACT_VALIDATION]: (data, id, start) => postResult(OUT_TYPES.CONTACT_VALIDATED, processContactValidation(data), id, start),
    [MESSAGE_TYPES.OPTIMIZE_IMAGES]: (data, id, start) => postResult(OUT_TYPES.IMAGES_OPTIMIZED, processImageOptimization(data), id, start),
    [MESSAGE_TYPES.GET_PERFORMANCE_STATS]: (_data, id) => self.postMessage({
...
function processAnimationData(data) {
    ...
    const processed = elements.map((element) => {
        ...
        const elementProgress = Math.max(0, Math.min(1, (scrollProgress - startOffset) / (endOffset - startOffset)));
        ...
        return Object.assign(Object.assign({}, element), { progress: easedProgress, properties: computedProperties, isVisible: elementProgress > 0 && elementProgress < 1 });
    });
...
function optimizeScrollCalculations(data) {
    ...
    return elements.map((element) => {
        ...
        return Object.assign(Object.assign({}, element), { isVisible: intersectionRatio > threshold, intersectionRatio,
...
function calculatePerformanceMetrics(data) {
    ...
    const metrics = {
        ...
        fcp: (paintTiming === null || paintTiming === void 0 ? void 0 : paintTiming["first-contentful-paint"]) || 0,
        lcp: (paintTiming === null || paintTiming === void 0 ? void 0 : paintTiming["largest-contentful-paint"]) || 0,
        // Navigation timing metrics
        domContentLoaded: ((_a = navigationTiming === null || navigationTiming === void 0 ? void 0 : navigationTiming.domContentLoadedEventEnd) !== null && _a !== void 0 ? _a : 0) - ((_b = navigationTiming === null || navigationTiming === void 0 ? void 0 : navigationTiming.navigationStart) !== null && _b !== void 0 ? _b : 0),
        loadComplete: ((_c = navigationTiming === null || navigationTiming === void 0 ? void 0 : navigationTiming.loadEventEnd) !== null && _c !== void 0 ? _c : 0) - ((_d = navigationTiming === null || navigationTiming === void 0 ? void 0 : navigationTiming.navigationStart) !== null && _d !== void 0 ? _d : 0),
        // Resource loading analysis
        totalResources: (resourceTiming === null || resourceTiming === void 0 ? void 0 : resourceTiming.length) || 0,
        slowResources: (resourceTiming === null || resourceTiming === void 0 ? void 0 : resourceTiming.filter((r) => r.duration > SLOW_RESOURCE_LIMIT).length) || 0,
        // Performance score estimation
        performanceScore: 0,
    };
    // Calculate simple performance score
    let score = 100;
    if (metrics.fcp > PERF_FCP_LIMIT)
        score -= 20;
    if (metrics.lcp > PERF_LCP_LIMIT)
        score -= 25;
    if (metrics.domContentLoaded > PERF_DCL_LIMIT)
        score -= 15;
    if (metrics.slowResources > 0)
        score -= metrics.slowResources * 5;
...
function processContactValidation(data) {
    const { fields } = data;
    const validation = {};
    const SAFE_EMAIL_REGEX = /^[A-Za-z0-9.!#$%&'*+/=?^_`{|}~-]{1,64}@[A-Za-z0-9-]{1,63}(?:\.[A-Za-z0-9-]{1,63})+$/;
        ...
        switch (fieldName) {
            ...
            case "name":
                validation[fieldName] = {
                    ...
                    message: String(value).trim().length >= 2 ? "" : "Name must be at least 2 characters long",
                };
                break;
            case "message":
                validation[fieldName] = {
                    isValid: String(value).trim().length >= 10,
                    message: String(value).trim().length >= 10 ? "" : "Message must be at least 10 characters long",
                };
                break;
            default:
...
function processImageOptimization(data) {
    ...
    return images.map((image) => {
        ...
        return Object.assign(Object.assign({}, image), { optimizedSizes, estimatedSavings: calculateImageSavings(format, width, height), srcset: optimizedSizes
...
setInterval(() => {
    ...
    self.postMessage({
        ...
        data: Object.assign(Object.assign({}, workerState.performanceMetrics), { cacheSize: workerState.cache.size, memoryUsage: mem }),
    });
